...
function bind2(pa, apb) {
  return (input, cursor = CURSOR) =>
    bind(
      pa(input, cursor),
      (ok2) => apb(ok2.result)(ok2.remainder, ok2.cursor),
    );
}
...
function map3(pa, pb, pc, abcd) {
  return bind2(
    pa,
    (a) => bind2(pb, (b) => bind2(pc, (c) => pure(abcd(a, b, c)))),
  );
}
function left(pa, pb) {
...
function expr() {
  return (input, cursor = CURSOR) =>
    map22(
      ...
      zeroOrMore(call()),
      (value2, calls) =>
        calls.reduce((expr2, { ident: ident2, args }) => ({
          kind: "call",
          ident: ident2,
          args: [
            expr2,
    ...
    right(
      ...
      oneOf(
        ...
        map22(
          ...
          wrap(
            "(",
            map3(trim(expr()), char(","), trim(expr()), (e1, _, e2) => [
              e1,
              e2,
            ]),
            ")",
...
function evalVars(vars, funcs, varmap = []) {
  return vars.reduce(
    (varmapres, { ident: ident2, expr: expr2 }) =>
      bind(
        varmapres,
        (varmap2) =>
          map(evalExpr(expr2, varmap2, funcs), (value2) => [
            [
              ident2,
              value2,
            ],
...
function evalFunc(ident2, args, varmap, funcs) {
  ...
  const func = funcs.find((func2) =>
    func2.ident === ident2 && func2.params.length === arity
  );
  if (!func) {
  ...
  return bind(
    ...
    (varmap2) => evalExpr(func.scope.expr, varmap2, funcs),
  );
}
function evalBuiltIn(ident2, exprs2, varmap, funcs) {
  ...
  return bind(
    mapM(exprs2.map((expr2) => evalExpr(expr2, varmap, funcs))),
    (values) => {
      ...
      if (arity === 2) {
        if (t1 === "number" && t2 === "number") {
          ...
          const name = values[1];
          const func = funcs.find((func2) =>
            func2.ident === name && func2.params.length === 1
          );
          if (func) {
            return ok({
              kind: "stream",
          ...
          const num = values[1];
          return Array(num - 1).fill(0).reduce(
            (acc, _) =>
              bind(acc, (val) =>
                evalFunc(
                  func.ident,
